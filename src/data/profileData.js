export const profileUser = {
  name: "user1",
  handle: "@user1",
  initials: "JD",
  bio: "Logging albums I spin - mostly alternative, R&B, and whatever's on repeat this week.",
  location: "Makati, PH",
  joined: "Joined Feb 2026",
};

export const favoriteAlbums = [
  {
    title: "clepsydra",
    artist: "chouchou merged syrups.",
    artistId: "chouchou",
    releaseId: "d2e42e38-f0a5-4750-8c5d-7eddf26287b8",
    year: "2014",
    cover: "https://f4.bcbits.com/img/a0787078428_10.jpg",
    rating: 4.5,
  },
  {
    title: "Love Is Here",
    artist: "Starsailor",
    artistId: "starsailor",
    year: "2001",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/8/84/Love_is_Here_Starsailor.jpg/250px-Love_is_Here_Starsailor.jpg",
    rating: 4,
  },
  {
    title: "The French Operation",
    artist: "GIRLS BE",
    artistId: "girlsbe",
    year: "1997",
    cover: "https://media.senscritique.com/media/000022618518/300/the_french_operation.jpg",
    rating: 4,
  },
  {
    title: "The New Abnormal",
    artist: "The Strokes",
    artistId: "strokes",
    year: "2020",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/f/f8/The_Strokes_-_The_New_Abnormal.png/250px-The_Strokes_-_The_New_Abnormal.png",
    rating: 4.5,
  },
  {
    title: "0",
    artist: "Low Roar",
    artistId: "lowroar",
    year: "2014",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/a/a8/Low_Roar_%270%27_album_cover_art.jpg/250px-Low_Roar_%270%27_album_cover_art.jpg",
    rating: 4,
  },
];

export const recentCarouselAlbums = [
  {
    title: "Norman F. Rockwell!",
    artist: "Lana Del Rey",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/8/8a/Lana_Del_Rey_-_Norman_Fucking_Rockwell.png/250px-Lana_Del_Rey_-_Norman_Fucking_Rockwell.png",
    rating: 4.5,
  },
  {
    title: "MOTOMAMI",
    artist: "Rosalia",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/9/9c/Rosal%C3%ADa_-_Motomami.png/250px-Rosal%C3%ADa_-_Motomami.png",
    rating: 4,
  },
  {
    title: "Mama's Gun",
    artist: "Erykah Badu",
    cover: "https://upload.wikimedia.org/wikipedia/en/thumb/a/ab/Mama%27s_Gun.png/250px-Mama%27s_Gun.png",
    rating: 4.5,
  },
  {
    title: "AM",
    artist: "Arctic Monkeys",
    cover: "https://upload.wikimedia.org/wikipedia/en/c/cd/Arctic_Monkeys_-_AM.png",
    rating: 4,
  },
  {
    title: "Jubilee",
    artist: "Japanese Breakfast",
    cover: "https://upload.wikimedia.org/wikipedia/en/5/5d/Japanese_Breakfast_-_Jubilee.png",
    rating: 3.5,
  },
];

export const recentLogs = [
  {
    title: "Hounds of Love",
    artist: "Kate Bush",
    year: "1985",
    rating: "4.5",
    note: "That run from track 1-5 is unreal.",
    timeAgo: "2h ago",
  },
  {
    title: "Currents",
    artist: "Tame Impala",
    year: "2015",
    rating: "4.0",
    note: "Perfect late-night commute album.",
    timeAgo: "1d ago",
  },
  {
    title: "SOS",
    artist: "SZA",
    year: "2022",
    rating: "3.5",
    note: "Some skips, but the highs are HIGH.",
    timeAgo: "3d ago",
  },
];

const allFriends = [
  {
    name: "Ari Santos",
    handle: "@aris",
    slug: "aris",
    initials: "AS",
    avatarSrc: "https://i.pravatar.cc/160?u=aris",
    bannerSrc: "/hero/hero1.jpg",
    lastfmUsername: "arisfm",
    activity: "Now spinning",
    note: "SZA - SOS",
    location: "Quezon City, PH",
    joined: "Joined Jan 2026",
    bio: "Night-shift playlist curator. Rates albums by replay value and sequencing.",
    stats: {
      logs: 184,
      averageRating: 4.2,
      streakDays: 16,
    },
    favoriteAlbums: [
      {
        title: "SOS",
        artist: "SZA",
        year: "2022",
        cover: "/album/blond.jpg",
      },
      {
        title: "To Pimp a Butterfly",
        artist: "Kendrick Lamar",
        year: "2015",
        cover: "/album/tpab.jpg",
      },
      {
        title: "AM",
        artist: "Arctic Monkeys",
        year: "2013",
        cover: "/album/am.jpg",
      },
    ],
    recentLogs: [
      {
        title: "SOS",
        artist: "SZA",
        year: "2022",
        rating: 4.5,
        timeAgo: "34m ago",
      },
      {
        title: "Currents",
        artist: "Tame Impala",
        year: "2015",
        rating: 4.0,
        timeAgo: "Yesterday",
      },
      {
        title: "Blonde",
        artist: "Frank Ocean",
        year: "2016",
        rating: 5.0,
        timeAgo: "2d ago",
      },
    ],
  },
  {
    name: "Mina Cruz",
    handle: "@minac",
    slug: "minac",
    initials: "MC",
    avatarSrc: "https://i.pravatar.cc/160?u=minac",
    bannerSrc: "/hero/hero2.jpg",
    lastfmUsername: "mina.crates",
    activity: "2h ago",
    note: "Saved 3 albums",
    location: "Taguig, PH",
    joined: "Joined Dec 2025",
    bio: "Collects dream-pop and late-2000s indie. Loves detail-heavy production.",
    stats: {
      logs: 142,
      averageRating: 4.0,
      streakDays: 9,
    },
    favoriteAlbums: [
      {
        title: "i,i",
        artist: "Bon Iver",
        year: "2019",
        cover: "/album/i,i.jpg",
      },
      {
        title: "Random Access Memories",
        artist: "Daft Punk",
        year: "2013",
        cover: "/album/ram.jpg",
      },
      {
        title: "IGOR",
        artist: "Tyler, The Creator",
        year: "2019",
        cover: "/album/igor.jpg",
      },
    ],
    recentLogs: [
      {
        title: "RAM",
        artist: "Daft Punk",
        year: "2013",
        rating: 4.5,
        timeAgo: "2h ago",
      },
      {
        title: "NFR!",
        artist: "Lana Del Rey",
        year: "2019",
        rating: 4.0,
        timeAgo: "1d ago",
      },
      {
        title: "AM",
        artist: "Arctic Monkeys",
        year: "2013",
        rating: 3.5,
        timeAgo: "2d ago",
      },
    ],
  },
  {
    name: "Leo Park",
    handle: "@leopark",
    slug: "leopark",
    initials: "LP",
    avatarSrc: "https://i.pravatar.cc/160?u=leopark",
    bannerSrc: "/hero/hero3.jpg",
    lastfmUsername: "leospins",
    activity: "Yesterday",
    note: "Rated 4.5",
    location: "Pasig, PH",
    joined: "Joined Nov 2025",
    bio: "Post-rock and ambient enthusiast. Writes short notes for every listen.",
    stats: {
      logs: 201,
      averageRating: 4.3,
      streakDays: 22,
    },
    favoriteAlbums: [
      {
        title: "AM",
        artist: "Arctic Monkeys",
        year: "2013",
        cover: "/album/am.jpg",
      },
      {
        title: "Blonde",
        artist: "Frank Ocean",
        year: "2016",
        cover: "/album/blond.jpg",
      },
      {
        title: "Currents",
        artist: "Tame Impala",
        year: "2015",
        cover: "/album/currents.jpg",
      },
    ],
    recentLogs: [
      {
        title: "The New Abnormal",
        artist: "The Strokes",
        year: "2020",
        rating: 4.5,
        timeAgo: "Yesterday",
      },
      {
        title: "0",
        artist: "Low Roar",
        year: "2014",
        rating: 4.0,
        timeAgo: "2d ago",
      },
      {
        title: "clepsydra",
        artist: "chouchou merged syrups.",
        year: "2014",
        rating: 4.5,
        timeAgo: "4d ago",
      },
    ],
  },
  {
    name: "Jules Reyes",
    handle: "@julesr",
    slug: "julesr",
    initials: "JR",
    avatarSrc: "https://i.pravatar.cc/160?u=julesr",
    bannerSrc: "/hero/hero1.jpg",
    lastfmUsername: "julesmix",
    activity: "3d ago",
    note: "Shared a list",
    location: "Cebu, PH",
    joined: "Joined Oct 2025",
    bio: "Playlist-first listener. Focuses on pop, R&B, and left-field electronic.",
    stats: {
      logs: 119,
      averageRating: 3.9,
      streakDays: 5,
    },
    favoriteAlbums: [
      {
        title: "IGOR",
        artist: "Tyler, The Creator",
        year: "2019",
        cover: "/album/igor.jpg",
      },
      {
        title: "To Pimp a Butterfly",
        artist: "Kendrick Lamar",
        year: "2015",
        cover: "/album/tpab.jpg",
      },
      {
        title: "i,i",
        artist: "Bon Iver",
        year: "2019",
        cover: "/album/i,i.jpg",
      },
    ],
    recentLogs: [
      {
        title: "MOTOMAMI",
        artist: "Rosalia",
        year: "2022",
        rating: 3.5,
        timeAgo: "3d ago",
      },
      {
        title: "Mama's Gun",
        artist: "Erykah Badu",
        year: "2000",
        rating: 4.0,
        timeAgo: "5d ago",
      },
      {
        title: "SOS",
        artist: "SZA",
        year: "2022",
        rating: 4.0,
        timeAgo: "6d ago",
      },
    ],
  },
  {
    name: "Nika Lim",
    handle: "@nikalim",
    slug: "nikalim",
    initials: "NL",
    avatarSrc: "https://i.pravatar.cc/160?u=nikalim",
    bannerSrc: "/hero/hero2.jpg",
    lastfmUsername: "nika_weekends",
    activity: "1w ago",
    note: "Listened to 12",
    location: "Davao, PH",
    joined: "Joined Sep 2025",
    bio: "Weekend crate-digger. Mostly alt-rock with occasional jazz detours.",
    stats: {
      logs: 167,
      averageRating: 4.1,
      streakDays: 11,
    },
    favoriteAlbums: [
      {
        title: "Random Access Memories",
        artist: "Daft Punk",
        year: "2013",
        cover: "/album/ram.jpg",
      },
      {
        title: "AM",
        artist: "Arctic Monkeys",
        year: "2013",
        cover: "/album/am.jpg",
      },
      {
        title: "Currents",
        artist: "Tame Impala",
        year: "2015",
        cover: "/album/currents.jpg",
      },
    ],
    recentLogs: [
      {
        title: "Titanic Rising",
        artist: "Weyes Blood",
        year: "2019",
        rating: 4.5,
        timeAgo: "1w ago",
      },
      {
        title: "Punisher",
        artist: "Phoebe Bridgers",
        year: "2020",
        rating: 4.0,
        timeAgo: "8d ago",
      },
      {
        title: "In Rainbows",
        artist: "Radiohead",
        year: "2007",
        rating: 5.0,
        timeAgo: "10d ago",
      },
    ],
  },
];

export const friends = allFriends.filter((friend) => friend.slug === "aris");

export const findFriendBySlug = (slug) => friends.find((friend) => friend.slug === slug);

export const reviews = [
  {
    title: "Titanic Rising",
    artist: "Weyes Blood",
    year: "2019",
    rating: 4.5,
    summary: "A celestial slow burn with the best kind of soft-focus drama.",
    timeAgo: "2d ago",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/2/2c/Weyes_Blood_-_Titanic_Rising.png",
  },
  {
    title: "Punisher",
    artist: "Phoebe Bridgers",
    year: "2020",
    rating: 4,
    summary: "Spare, sharp, and oddly comforting. Every lyric lands.",
    timeAgo: "5d ago",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/4/47/Phoebe_Bridgers_-_Punisher.png/220px-Phoebe_Bridgers_-_Punisher.png",
  },
  {
    title: "In Rainbows",
    artist: "Radiohead",
    year: "2007",
    rating: 5,
    summary: "The warmest Radiohead record, still impossibly detailed.",
    timeAgo: "1w ago",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/2/2e/In_Rainbows_Official_Cover.jpg",
  },
  {
    title: "Cavalcade",
    artist: "black midi",
    year: "2021",
    rating: 3.5,
    summary: "Chaotic and theatrical. A little exhausting, mostly thrilling.",
    timeAgo: "2w ago",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/2/21/Black_Midi_-_Cavalcade.png/220px-Black_Midi_-_Cavalcade.png",
  },
];

export const recentReviews = [
  {
    title: "Desire, I Want to Turn Into You",
    artist: "Caroline Polachek",
    year: "2023",
    rating: 4.5,
    summary: "Fluent, glamorous pop with zero filler.",
    timeAgo: "4h ago",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/7/7b/Caroline_Polachek_-_Desire%2C_I_Want_to_Turn_Into_You.png/220px-Caroline_Polachek_-_Desire%2C_I_Want_to_Turn_Into_You.png",
  },
  {
    title: "Blonde",
    artist: "Frank Ocean",
    year: "2016",
    rating: 4,
    summary: "Still feels like a diary you are not supposed to read.",
    timeAgo: "1d ago",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/a/a0/Blonde_-_Frank_Ocean.jpeg/220px-Blonde_-_Frank_Ocean.jpeg",
  },
  {
    title: "Fetch the Bolt Cutters",
    artist: "Fiona Apple",
    year: "2020",
    rating: 4.5,
    summary: "Ferocious, funny, and incredibly alive.",
    timeAgo: "3d ago",
    cover:
      "https://upload.wikimedia.org/wikipedia/en/thumb/3/32/Fiona_Apple_-_Fetch_the_Bolt_Cutters.png/220px-Fiona_Apple_-_Fetch_the_Bolt_Cutters.png",
  },
];
